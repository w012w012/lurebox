...
/// 支持多种开源视觉模型
class SiliconFlowFishRecognitionProvider implements FishRecognitionProvider {
  /// 系统提示词 - 用于指导模型识别鱼类物种
...
请直接返回 JSON，不要包含其他文字说明。''';
  ...
  Future<FishRecognitionResult> identifySpecies(
    ...
    final requestBody = {
      'model': config.modelName ?? 'Qwen/Qwen2-VL-72B-Instruct',
      'messages': [
        {
          'role': 'system',
    ...
    // 构建请求 URL - SiliconFlow API 端点
    final baseUrl = config.baseUrl ?? 'https://api.siliconflow.cn';
