...
请直接返回 JSON，不要包含其他文字说明。''';
  ...
  Future<FishRecognitionResult> identifySpecies(
    ...
    debugPrint('[CustomProvider] API Key length: ${config.apiKey.length}');
    debugPrint(
        '[CustomProvider] API Key first 10 chars: ${config.apiKey.substring(0, config.apiKey.length > 10 ? 10 : config.apiKey.length)}...');
    debugPrint(
        '[CustomProvider] API Key last 10 chars: ...${config.apiKey.substring(config.apiKey.length > 10 ? config.apiKey.length - 10 : 0)}');
    debugPrint(
        '[CustomProvider] API Key has whitespace: ${config.apiKey.contains(RegExp(r'\s'))}');

    try {
      // 发送请求，设置 60 秒超时（视觉模型处理图片需要更长时间）
  ...
  FishRecognitionResult _handleResponse(http.Response response, String apiKey) {
    ...
    switch (response.statusCode) {
      ...
      case 401:
        ...
        debugPrint('[CustomProvider] $keyInfo');
        throw FishRecognitionException(
          FishRecognitionErrorType.apiKeyInvalid,
          'API 密钥无效 (401)\n$keyInfo\nAPI返回: ${response.body}',
        );
